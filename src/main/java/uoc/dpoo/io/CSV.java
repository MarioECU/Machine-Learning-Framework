...
public class CSV implements Cloneable{
	 ...
	 * @throws Exception Raised when there are a problem reading the file or
	...
	/**
	 * Private constructor, used to clone the object. Only calls load when features
	 * is empty
	 * 
	 * @param path          Path where the CSV is stored.
	 * @param sep           Separator used to split the lines into features.
	...
	private void load() throws Exception {
		...
		// NOTA: Si linea anterior causa errores en entornos Windows, SUBSTITUIR por
		// esta:
		// CSVReader reader = new CSVReaderBuilder(Files.newBufferedReader(Paths.get(new
		// File(path).toURI()))).withCSVParser(parser).build();
	...
	/**
	 * Reads the columns names and store the values in the instance. For each column
	 * create the key and add a new Feature.
	 * 
	...
	/**
	 * Reads all values in CSV file and store them in the instance. For each line in
	 * CSV. - Check the value is in missingValues and map it to null. - Insert the
	 * value in values.
	 * 
	 * @param reader CSVReader used to read the CSV
	 * @throws Exception Raised when there are a problem reading the file or
	...
	/**
	 * Return a list with all values where all the missing values are converted to
	 * null.
	 * 
	...
	public List<String> getColumnsNames() {
		return new ArrayList<String>(features.keySet()).stream().sorted().collect(Collectors.toList());
	}
	...
	/**
	 * Add or update a feature. This method should update features with a new or
	 * updated feature
	 * 
	...
	/**
	 * Get a feature identified by its name. If the name is not in the CSV an
	 * CSVException is raised.
	 * 
	...
	/**
	 * Returns a CSV row identified by it´s position. The row starts from 0, to
	 * rowNumbers, an exception is raised if the row is out of bound.
	 * 
	 * @param row Row to extract the value
	 * @throws CSVException Raised when row less than 0 or greater than rowsNumber.
	 * @return A hashMap where the key is the name of the feature and the value the
	 *         value of the feature in the row
	 */
	 ...
	 * @param suffix Suffix to used when create the file. Per example "training",
	 *               "removedNA".
	...
	public String write(char sep, String suffix) throws IOException {
		...
		CSVWriter writer = new CSVWriter(new FileWriter(outputPath), sep, CSVWriter.NO_QUOTE_CHARACTER,
