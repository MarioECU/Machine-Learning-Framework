...
public class Feature implements Cloneable {
	...
	/**
	 * Checks if all the values are numbers and change the features' type If the
	 * feature has all values as number then the type is FeatureType.NUMBER if is
	 * not a number or the values is empty the type is FeatureType.OTHER
	 */
	public void fixType() {
		if (isNumber())
	...
	/**
	 * Check if all the values are numbers, in that case returns true , otherwise
	 * false. Before checks the values, filter the nulls
	 * 
